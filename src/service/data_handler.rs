use crate::{
	...
	utils::constants::{self, errors, juno_hooks, request_keys, request_types},
};
...
lazy_static! {
	static ref REGISTERED_MODULES: RwLock<HashMap<String, Module>> =
		RwLock::new(HashMap::new());
	static ref UNREGISTERED_MODULES: RwLock<HashMap<String, Module>> =
		RwLock::new(HashMap::new());
	static ref REQUEST_ORIGINS: RwLock<HashMap<String, String>> =
		RwLock::new(HashMap::new());
	static ref MODULE_UUID_TO_ID: RwLock<HashMap<u128, String>> =
		RwLock::new(HashMap::new());
}

pub async fn handle_request(module_comm: &ModuleComm, data: String) {
	let json_result = serde_json::from_str(&data);
	log::trace!("Got request. Processing...");

	if json_result.is_err() {
		log::warn!("Request is not parsable. Ignoring...");
		return;
	}
	...
	log::trace!("Completed processing the request");
}
...
pub async fn on_module_disconnected(module_comm: &ModuleComm) {
	...
	if module_id.is_none() {
		log::trace!(
			"Module does not have a moduleId. No more processing required"
		);
		return;
	}
	let module_id = module_id.unwrap();
	log::trace!("Module is associated with moduleId '{}'", module_id);

	let mut registered_modules = REGISTERED_MODULES.write().await;
	let mut unregistered_modules = UNREGISTERED_MODULES.write().await;

	if registered_modules.contains_key(&module_id) {
		log::trace!("Module is a registered module. Removing...");
		registered_modules
			.remove(&module_id)
			.unwrap()
			.close_sender()
			.await;
		log::trace!("Module removed from registered modules");
	} else if unregistered_modules.contains_key(&module_id) {
		log::trace!("Module is an registered module. Removing...");
		unregistered_modules
			.remove(&module_id)
			.unwrap()
			.close_sender()
			.await;
		log::trace!("Module removed from unregistered modules");
	}
	drop(registered_modules);
	drop(unregistered_modules);
	log::info!("Module '{}' disconnected.", module_id);
	...
	log::trace!("Module is no longer tracked");
}
...
async fn handle_module_registration(
	module_comm: &ModuleComm,
	request_id: &str,
	request: &Value,
) {
	...
	if module_id == None {
		log::debug!("moduleId not present. Sending error...");
	...
	if version == None {
		log::debug!("version not present. Sending error...");
		send_error(module_comm, request_id, errors::MALFORMED_REQUEST).await;
		return;
	}
	...
	if let Some(dependencies) = dependencies {
		log::trace!("Dependency is not null. Populating hashmap...");
		for dependency in dependencies.keys() {
			if !dependencies[dependency].is_string() {
				...
				send_error(module_comm, request_id, errors::MALFORMED_REQUEST)
					.await;
				return;
			}
			...
			if dependency_requirement.is_err() {
				log::debug!("Dependency value for key {} is not a valid SemVer version requirement. Sending error...", dependency);
				send_error(module_comm, request_id, errors::MALFORMED_REQUEST)
					.await;
				return;
			}
			dependency_map
				.insert(dependency.clone(), dependency_requirement.unwrap());
		}
		log::trace!(
		...
		log::trace!("Dependency is null. Pre-assigning a new HashMap");
	}

	let version = Version::parse(version);
	if version.is_err() {
		log::debug!("version not valid. Sending error...");
		send_error(module_comm, request_id, errors::MALFORMED_REQUEST).await;
		return;
	}
	let version = version.unwrap();
	...
	if registered_modules.contains_key(module_id)
		|| unregistered_modules.contains_key(module_id)
	{
		log::debug!("Either registered modules or unregistered modules already has this moduleId. Sending error...");
	...
	if module_uuid_to_id.contains_key(module_comm.get_uuid()) {
		log::debug!("A moduleId for that UUID already exists. This looks like a duplicate module. Sending error...");
		send_error(module_comm, request_id, errors::DUPLICATE_MODULE).await;
		return;
	}
	...
	log::trace!("Notifying successful module registration...");
	send_module(
		&module,
		&json!({
	...
	.await;
	log::trace!("Notification successful");

	if module.get_dependencies().is_empty() {
		module.set_registered(true);

		log::trace!("Triggering activation hook...");
		trigger_hook_on(
			constants::APP_NAME,
			&module,
		...
		.await;
		log::trace!("Activation hook triggered");

		log::trace!("Dependencies are none. Adding to registered modules...");
		registered_modules.insert(String::from(module_id), module);
		log::trace!("Module added to registered modules");

		drop(registered_modules);
		drop(unregistered_modules);

		log::trace!("Notifying all modules of activated module...");
		// let juno_module = REGISTERED_MODULES
		// 	.read()
		// 	.await
		// 	.get(constants::APP_NAME)
		...
		// )
		// .await;
		log::trace!("All modules notified of activated module");
	} else {
		module.set_registered(false);

		log::trace!(
			"Module has dependencies. Adding to unregistered modules..."
		);
		unregistered_modules.insert(String::from(module_id), module);
		log::trace!("Module added to unregistered modules");
...
async fn handle_declare_function(
	module_comm: &ModuleComm,
	request_id: &str,
	request: &Value,
) {
	...
	if module_id.is_none() {
		log::debug!("moduleId not found. Sending error...");
	...
	if !registered_modules.contains_key(&module_id) {
		log::debug!("This module is not registered. Sending error...");
		send_error(module_comm, request_id, errors::UNREGISTERED_MODULE).await;
		return;
	}
	...
	if function.is_none() {
		log::debug!("Function is not parsable as a string. Sending error...");
		send_error(module_comm, request_id, errors::MALFORMED_REQUEST).await;
		return;
	}
	...
	if !module.is_function_declared(&function) {
		...
		log::warn!(
			"This function is already declared. No need to register it again"
		);
	}

	log::trace!("Informing module of successful function declaration...");
	send_module(
		module,
		&json!({
			request_keys::REQUEST_ID: request_id,
	...
	.await;
	log::trace!("Success response has been sent");
}
...
async fn handle_function_call(
	...
	if function_name.is_none() {
		log::debug!("Function is not of the format 'module-name.function_name'. Sending error...",);
	...
	if request_origins.contains_key(&request_id_heap) {
		if request_origins[&request_id_heap] != module_id {
			...
			log::error!("The call to function '{}' had a requestId '{}', which is already declared. In order to prevent request hijacking, this request will be errored. Sending error...", function, request_id);
			send_error(module_comm, request_id, errors::INVALID_REQUEST_ID)
				.await;
			return;
		} else {
			log::debug!("There already seems to be a request to module '{}' with the requestId '{}'. This may or may not be intended. Are you sending the same request twice?", module_name, request_id);
		}
	} else {
		log::trace!("Registering the requestId along with it's origin module.");
		request_origins.insert(request_id_heap, module_id.clone());
	}
	drop(request_origins);

	let mut response = request.clone();
	log::trace!("Cloning request to send as response");
	if request[request_keys::ARGUMENTS].as_object() == None {
		log::debug!("The call to function had no arguments (or the arguments were not an object). A new, empty object will be assigned");
		response[request_keys::ARGUMENTS] = Value::Object(Map::new());
	}

	log::trace!("Changing the value of function from 'module-name.function_name' to just 'function_name' in the request");
	response[request_keys::FUNCTION] = Value::String(function_name);

	log::trace!("Setting the caller of the function call...");
	response[request_keys::CALLER] = Value::String(module_id);

	log::trace!("Proxying the request to the relevant module...");
	send_module(receiver_module, &response).await;
	log::trace!("Function call proxied.");
}
...
async fn handle_function_response(
	...
	if !request_origins.contains_key(request_id) {
		...
		log::error!("The function response with requestId '{}' does not contain an origin. The response might be malformed. Please ensure the function response has the same requestId as the function call", request_id);
		send_error(module_comm, request_id, errors::MALFORMED_REQUEST).await;
		return;
	}
	...
	if !registered_modules.contains_key(&origin_module_id) {
		...
		// Drop the packet entirely
		log::debug!("The function response meant for module '{}' is not registered (is the module still connected?). This packet will be ignored.", origin_module_id);
		return;
	}
	...
	send_module(registered_modules.get(&origin_module_id).unwrap(), request)
		.await;
	log::trace!("Function response to origin module successfully sent.");
}
...
async fn handle_register_hook(
	...
	if hook == None {
		log::debug!("Hook is not parsable as a string. Sending error...");
		send_error(module_comm, request_id, errors::MALFORMED_REQUEST).await;
		return;
	}
	...
	log::trace!("Hook registered. Sending success response to module...");
	send_module(
		module,
		&json!({
			request_keys::REQUEST_ID: request_id,
	...
	.await;
	log::info!("Hook registration done, and success response has been sent");
}
...
async fn handle_trigger_hook(
	...
	let data = if data == None {
		log::debug!("The triggered hook had no arguments (or the arguments were not an object). A new, empty object will be assigned");
		Map::new()
	} else {
		log::trace!("The hook data will be cloned to send to modules");
		data.unwrap().clone()
	};
	...
	.await;
	log::trace!("Origin module has been notified of hook triggered");
}
...
async fn trigger_hook(
	...
	log::trace!("Iterating all registered modules to send hook to...");
	...
	log::trace!("All registered modules have been processed");

	if sticky {
		log::info!(
			"This hook is being stickied. Saving it for future modules..."
		);
		log::warn!("TODO stickying hooks is not implemented yet");
	} else {
		log::trace!("This hook is not being stickied.");
	}
}
...
async fn trigger_hook_on(
	...
	if force {
		log::trace!("Hook is being forced onto the module...");
		send_module(
			to_module,
			&json!({
				request_keys::REQUEST_ID: generate_request_id().await,
		...
		log::trace!("The module is registered for the hook. Sending hook...");
		send_module(
			to_module,
			&json!({
				request_keys::REQUEST_ID: generate_request_id().await,
	...
	log::trace!("Hook sent to module.");
}

async fn recalculate_all_module_dependencies() {
	log::trace!("Recalculating all module dependencies...");
	...
	log::trace!("Checking if any unregistered modules are satisfied...");
	...
	for (module_id, module) in unregistered_modules.iter() {
		...
		log::trace!("Checking dependencies for module '{}'", module_id);
	...
	for module_id in satisfied_modules.iter() {
		...
		.await;
		log::trace!("ACTIVATED trigger sent.");

		log::trace!("Adding module to registered_modules...");
		registered_modules.insert(module_id.clone(), module);
		log::trace!("Module registered");
	}
	log::trace!("All newly satisfied modules registered");
	drop(registered_modules);
	drop(unregistered_modules);
	...
	// 	.unwrap()
	// 	.clone();
	// log::trace!("Notifying all modules of activated modules...");
	...
	// 	.await;
	// }
	log::trace!("All modules notified of activated modules");
	...
	log::trace!(
		"Checking if any registered modules are no longer satisfied..."
	);
	...
	log::trace!("Unregistering all modules that are no longer satisfied...");
	...
	for module_id in unsatisfied_modules.iter() {
		...
		log::trace!("Sending DEACTIVATED trigger to module...");
		trigger_hook_on(
			constants::APP_NAME,
			&module,
		...
		.await;
		log::trace!("DEACTIVATED trigger sent");

		log::trace!("Adding module to unregistered_modules...");
		unregistered_modules.insert(module_id.clone(), module);
		log::trace!("Module unregistered");
	}
	log::trace!("All module whose dependencies are no longer satisfied are unregistered");
	drop(registered_modules);
	drop(unregistered_modules);

	log::trace!("Notifying all modules of deactivated modules...");
	...
	// 	.await;
	// }
	log::trace!("All modules notified of deactivated modules");

	log::trace!("All module dependencies recalculated");
}
...
async fn send_error(
	...
	log::trace!("{} error sent", error_name);
}
